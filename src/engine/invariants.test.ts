...
import { BALL_TYPES, isPlausible, validOutcomes, type PlayInput } from '../field/play';
...
const label = (p: PlayInput) =>
  `${p.situation.level} ${p.ball}/${p.outcome} at (${p.at.x.toFixed(0)}, ${p.at.y.toFixed(0)}) [${p.situation.posture}]`;
...
describe(`resolver invariants over ${PLAYS.length} plays`, () => {
  ...
  it('backs up every base that takes a throw', () => {
    const failures: string[] = [];
    for (const p of PLAYS) {
      const play = resolvePlay(p);
      const backed = new Set(
        play.assignments.map((a) => roleKey(a.role)).filter((k) => k.startsWith('backup:')),
      );
      for (const t of play.throws) {
        if (!backed.has(`backup:${t.to}`)) {
          failures.push(`${label(p)}: throw to ${t.to} has no backup`);
        }
      }
    }
    assertNoFailures(failures);
  ...
  it('sends the primary fielder to the ball', () => {
    const failures: string[] = [];
    for (const p of PLAYS) {
      const primary = resolvePlay(p).assignments.find((a) => a.role.kind === 'primary');
      if (!primary) {
        failures.push(`${label(p)}: no primary fielder`);
      } else if (dist(primary.target, p.at) > 0.01) {
